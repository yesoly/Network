...
class User:
    ...
    def fileHandler(self, nickname, filename, filedata, sendtype):
        # fsend
        if sendtype == 'all':
            sender_conn, sender_addr = self.users[nickname]
            for receiver_name, (conn, addr) in self.users.items():
                if conn == sender_conn and addr == sender_addr:
                    continue
                else:
                    self.sendSender('>> nickname %s is sending file %s' %(nickname, filename), receiver_name)
                    time.sleep(0.1)
                    sendname = 'fsend_' + nickname + '_' + receiver_name + '_' + filename
                    conn.send(('\\file ' + sendname).encode())
                    filedata = filedata.encode()
                    time.sleep(0.1)
                    while(len(filedata) > KB):
                        conn.send(filedata[:KB])
                        filedata = filedata[KB:]
                    conn.send(filedata)
                    time.sleep(0.1)
        ...
        # wsend
            ...
            receiver_name = sendtype
            conn, addr = self.users[receiver_name]
            sendname = 'wsend_' + nickname + '_' + receiver_name + '_' + filename
            conn.send(('\\file ' + sendname).encode())
            time.sleep(0.1)
            while(len(filedata) > KB):
                conn.send(filedata[:KB])
                ...
                if msg[0] == '\\':
                    ...
                    while True:
                        ...
                        if (f == '\\fin'):
                            ...
                            break
                        filedata += f
                    self.user.fileHandler(username, filename, filedata, sendtype)
                    handler_result = 0


                else:   
                    handler_result = self.user.messageHandler(username, msg)
